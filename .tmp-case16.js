...
const map_indexed = (f, lst) => ((() => { let indices = _fl_range(0, _fl_length(lst)); return _fl_map(indices, ((i) => f(i, _fl_get(lst, i)))); })())
const top_k_retrieval = (scored_list, k) => take(k, reverse(sort(scored_list, ((a, b) => (_fl_get(a, "score") < _fl_get(b, "score"))))))
const test_ai_ranking = () => ((() => { let scores = [({ idx: 0, score: 0.9 }), ({ idx: 1, score: 0.7 }), ({ idx: 2, score: 0.95 })]; return _fl_print([_fl_length(top_k_retrieval(scores, 2))]); })())
test_ai_ranking()