...
function cg_cond(args) { return ((_fl_length(args) === 0) ? "null" : cg_cond_loop(args, (_fl_length(args) - 1), "null")); }
function cg_cond_loop(args, i, acc) { return ((i < 0) ? acc : (() => { let clause = _fl_get(args, i); let citems = _fl_get(_fl_get(clause, "fields"), "items"); let test = cg(_fl_get(citems, 0)); let body = cg(_fl_get(citems, 1)); return cg_cond_loop(args, (i - 1), _fl_str("(", test, "?", body, ":", acc, ")")); })()); }
...
function compile_file(input, output) { return (() => { let src = _fl_file_read(input); let tokens = lex(src); let ast = parse(tokens); let js = fl__gtjs_with_prelude(ast); return (() => { _fl_file_write(output, js); _fl_print(_fl_str("📖 ", input, " -> ", output)); _fl_print(_fl_str("   Lexed: ", _fl_length(tokens), " tokens")); _fl_print(_fl_str("   Parsed: ", _fl_length(ast), " nodes")); return _fl_print(_fl_str("   Generated: ", _fl_length(js), " bytes")); })(); })(); }
...
function cli_main() { return (() => { let argv = _fl_get_argv(); return (_fl_null_q(argv) ? null : ((_fl_length(argv) === 0) ? null : (() => { let cmd = _fl_get(argv, 0); return ((cmd === "run") ? do_run(_fl_get(argv, 1)) : ((cmd === "compile") ? (() => { let input = _fl_get(argv, 1); let output = ((_fl_get(argv, 2) === "-o") ? _fl_get(argv, 3) : _fl_get(argv, 2)); return compile_file(input, output); })() : (true ? (() => { let input = _fl_get(argv, 0); let output = ((_fl_length(argv) >= 2) ? _fl_get(argv, 1) : _fl_str(input, ".out.js")); return compile_file(input, output); })() : null))); })())); })(); }
cli_main()
module.exports = {
