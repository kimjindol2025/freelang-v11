...
const BUILTIN_FUNCTIONS: Array<{ name: string; detail: string; doc: string }> = [
  ...
  { name: "group-by",       detail: "(group-by keyfn coll) -> map",        doc: "그룹핑: keyfn 결과값으로 coll을 분류. 빈 coll → {}" },
  ...
  { name: "safe-get",       detail: "(safe-get map key [default]) -> val",  doc: "nil-safe get: 2인자=nil이면 에러, 3인자=nil이면 default 반환" },
