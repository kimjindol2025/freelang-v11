...
describe("Rate Limiter (v11.6.20)", () => {
  ...
  test("기본 요청은 성공 (IP 초기 요청)", (done) => {
    const serverModule = createHttpServerModule(mockCallFn, mockCallFunctionValue);

    server = (serverModule.server_start as any)(
      port,
      () => ({ status: 200, body: "OK" })
    );
  ...
  test("시간 윈도우 초과 후 카운터 리셋", (done) => {
    ...
    // Rate limit: 1 요청 / 100ms (빠른 테스트)
    (serverModule.server_rate_limit as any)(1, 100);
