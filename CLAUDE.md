...
⭐⭐⭐ **[11절 코딩규칙 — 작업 순서, 협의, 검증, 보안, 롤백 (2026-05-15)](docs/CODING_RULES_11.md)**

**진화**: 9절 (2026-05-14) → 11절 (2026-05-15)
- 10절 추가: 보안 체크 (환경변수, 평문 저장 금지)
- 11절 추가: 롤백 기준 (검증 실패 2회 이상 즉시 롤백)
- 4절 수정: 줄 수 기본값 명시 (300줄 이하)

새 작업 시작 전 반드시 읽기. 리서치 → 플랜모드 → 문서화 → 태스크 등록 → 구현 → 검증.  
순서는 법이다. 이 규칙 없이는 실수가 반복된다.
