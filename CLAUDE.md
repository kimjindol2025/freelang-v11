...
### ⭐⭐⭐ [17절 코딩규칙 (2026-06-06)](docs/CODING_RULES_17.md)

**핵심**: 순서는 법이다. 리서치 → 플랜(토론) → SPEC.airc(잠금) → 승인 → 태스크 → 구현 → 검증 → … → 보고
...
| **1** | 리서치 | ⭐⭐⭐ |
| **2** | 플랜 (토론·휘발) | ⭐⭐ |
| **3** | **SPEC.airc 작성 (잠금)** ← 신규 | ⭐⭐⭐ |
| **4** | 사용자 승인 | ⭐⭐⭐ |
| **5** | 태스크 등록 (tasks.dclub.kr) | ⭐⭐ |
| **6** | 단락 구현 | ⭐⭐ |
| **7** | 단락 검증 (실제 명령) | ⭐⭐⭐ |
| **8** | 실패 시 수정 — PASS 전 다음 X (게이트) | ⭐⭐⭐ |
| **9** | 진행 기록 | ⭐⭐ |
| **10** | 미완료 시 6번 반복 | ⭐⭐ |
| **11** | 완료 선언 | ⭐ |
...
**진화**: 9절(05-14) → 11절(05-15) → 16절 → **17절(06-06)**
- **신설**: 3절 SPEC.airc 분리 (플랜=토론/SPEC=잠금 — 노선 잠금 + 인계)
- **신설**: 8·13 이중 PASS 게이트 (깨진 채 다음 X / 깨진 채 저장 X)
- **신설**: 12 통합 검증 (단락 검증과 분리)
- 11절 정책 lock(Edit만·--force 금지·보안·블로그)은 횡단 계승

**새 작업 시작 전 필수 체크**:
```
[ ] 1. CLAUDE.md 읽음
[ ] 2. CODING_RULES_17.md 읽음 (링크 위)
[ ] 3. 관련 파일 검토
[ ] 4. tasks.dclub.kr 프로젝트 확인
[ ] 5. 플랜 토론 (필요 시)
[ ] 6. SPEC.airc 작성 (잠금) ← 구현 전 필수
[ ] 7. 사용자 승인 + 태스크 등록
[ ] 8. 구현 시작
```

**전체 가이드**: [docs/CODING_RULES_17.md](docs/CODING_RULES_17.md) (상세) · 이전: [11절](docs/CODING_RULES_11.md)
