...
> ⚠️ **최우선 규칙**: 작업 전 반드시 [`TRUTH_POLICY.md`](./TRUTH_POLICY.md)
> 읽기. 실측 증거 없는 "완료/작동/X%" 주장은 거짓으로 간주. 본 CLAUDE.md
> 와 충돌 시 TRUTH_POLICY 가 우선.

**상태** (2026-04-18, 실측)
- `package.json version` = `11.0.0`
- `bootstrap.js` = 1,124,340 bytes (단일 번들, TS→JS)
- 런타임: **Node.js v25 전용** (현 상태 의존. 셀프호스팅 완성 시 제거 예정)
- `npm test` = Test Suites 17/17, Tests 583/583 PASS (2026-04-18 세션 관측)
- 셀프호스팅 진척: `self/` 디렉토리, Phase 01~10 커밋 존재 (HEAD=968da6e 근접).
  - Phase 10 codegen POC: `fact(15)=1,307,674,368,000` 생성 JS 를 node 가
    실행하는 것은 관측됨.
  - 재현성(FL→JS 매 회 생성) 및 fixed-point 컴파일은 **미검증**.
- 위험: 과거 커밋 메시지·메모리에 "Phase N 완성 / X% 파리티" 등 미검증
  단정 다수. `TRUTH_POLICY.md` 준수로 재발 방지 중.

> 이 섹션 값은 실측 출력을 기반으로 한다. "완성/프로덕션 준비" 같은
> 단정 표현은 TRUTH_POLICY 에 의해 금지.
