...
class Parser {
        ...
        while (!this.isAtEnd()) {
            ...
            if (this.check(token_1.TokenType.LBracket)) {
                ...
                if (nextIdx < this.tokens.length) {
                    ...
                    const isBlockKeyword = nextToken.type === token_1.TokenType.Module || nextToken.type === token_1.TokenType.TypeClass || nextToken.type === token_1.TokenType.Instance || nextToken.type === token_1.TokenType.Page || nextToken.type === token_1.TokenType.Api || nextToken.type === token_1.TokenType.Component || nextToken.type === token_1.TokenType.Form || nextToken.type === token_1.TokenType.Route || nextToken.type === token_1.TokenType.Service || nextToken.type === token_1.TokenType.Controller || nextToken.type === token_1.TokenType.Guard || nextToken.type === token_1.TokenType.Model || nextToken.type === token_1.TokenType.Query || nextToken.type === token_1.TokenType.Migration || nextToken.type === token_1.TokenType.Repository || nextToken.type === token_1.TokenType.Database || nextToken.type === token_1.TokenType.Cached || nextToken.type === token_1.TokenType.Kafka || nextToken.type === token_1.TokenType.JWT || nextToken.type === token_1.TokenType.OAuth || nextToken.type === token_1.TokenType.Dockerfile || nextToken.type === token_1.TokenType.K8sDeployment || nextToken.type === token_1.TokenType.AWS || nextToken.type === token_1.TokenType.GCP || nextToken.type === token_1.TokenType.Azure;
                    // 블록 타입은 반드시 UPPERCASE여야 함 (소문자 page/rule/form 등은 일반 변수)
                    const isKnownBlockType = nextToken.type === token_1.TokenType.Symbol && knownBlockTypes.includes(nextToken.value);
    ...
    parseBlock() {
        ...
        else if (typeToken.type === token_1.TokenType.Page) {
            blockType = "PAGE";
        }
        else if (typeToken.type === token_1.TokenType.Api) {
            blockType = "API";
        }
        else if (typeToken.type === token_1.TokenType.Route) {
            blockType = "ROUTE";
        }
        else if (typeToken.type === token_1.TokenType.Component) {
            blockType = "COMPONENT";
        }
        else if (typeToken.type === token_1.TokenType.Form) {
            blockType = "FORM";
        }
        else if (typeToken.type === token_1.TokenType.Service) {
            blockType = "SERVICE";
        }
        else if (typeToken.type === token_1.TokenType.Controller) {
            blockType = "CONTROLLER";
        }
        else if (typeToken.type === token_1.TokenType.Guard) {
            blockType = "GUARD";
        }
        else if (typeToken.type === token_1.TokenType.Model) {
            blockType = "MODEL";
        }
        else {
        ...
        let blockName;
        if (blockType === "FUNC" && (this.check(token_1.TokenType.Colon) || this.check(token_1.TokenType.Keyword))) {
    ...
    parseValue() {
        ...
        if (this.check(token_1.TokenType.String)) {
            ...
            if (token.value.includes("${")) {
                return (0, ast_1.makeTemplateString)(token.value);
            }
        ...
        if (this.check(token_1.TokenType.Variable)) {
            const token = this.advance();
            return (0, ast_1.makeVariable)(token.value, token.line);
        }
        ...
        if (this.check(token_1.TokenType.Colon)) {
            ...
            if (next) {
                ...
                if (next.type === token_1.TokenType.Variable && next.value) {
                    this.advance();
                    return (0, ast_1.makeLiteral)("string", "$" + next.value);
                }
                // :symbol, :keyword → "symbol"
                if (next.value && /^[a-zA-Z_][a-zA-Z0-9_\-]*$/.test(next.value)) {
                    return (0, ast_1.makeLiteral)("string", this.advance().value);
                }
            }
            // Bare colon
        ...
        if (this.check(token_1.TokenType.LBracket)) {
            ...
            else if (nextIdx < this.tokens.length && (this.tokens[nextIdx].type === token_1.TokenType.Page || this.tokens[nextIdx].type === token_1.TokenType.Api || this.tokens[nextIdx].type === token_1.TokenType.Component || this.tokens[nextIdx].type === token_1.TokenType.Form || this.tokens[nextIdx].type === token_1.TokenType.Route)) {
                // Phase 11 v11: PAGE/API/COMPONENT/FORM/ROUTE keyword tokens
                return this.parseBlock();
            }
            else if (nextIdx < this.tokens.length && (this.tokens[nextIdx].type === token_1.TokenType.Service || this.tokens[nextIdx].type === token_1.TokenType.Controller || this.tokens[nextIdx].type === token_1.TokenType.Guard || this.tokens[nextIdx].type === token_1.TokenType.Model || this.tokens[nextIdx].type === token_1.TokenType.Query || this.tokens[nextIdx].type === token_1.TokenType.Migration || this.tokens[nextIdx].type === token_1.TokenType.Repository || this.tokens[nextIdx].type === token_1.TokenType.Database || this.tokens[nextIdx].type === token_1.TokenType.Cached || this.tokens[nextIdx].type === token_1.TokenType.Kafka || this.tokens[nextIdx].type === token_1.TokenType.Producer || this.tokens[nextIdx].type === token_1.TokenType.Consumer || this.tokens[nextIdx].type === token_1.TokenType.Queue || this.tokens[nextIdx].type === token_1.TokenType.RabbitMQ || this.tokens[nextIdx].type === token_1.TokenType.JWT || this.tokens[nextIdx].type === token_1.TokenType.OAuth || this.tokens[nextIdx].type === token_1.TokenType.Dockerfile || this.tokens[nextIdx].type === token_1.TokenType.DockerCompose || this.tokens[nextIdx].type === token_1.TokenType.K8sDeployment || this.tokens[nextIdx].type === token_1.TokenType.K8sService || this.tokens[nextIdx].type === token_1.TokenType.K8sIngress || this.tokens[nextIdx].type === token_1.TokenType.AWS || this.tokens[nextIdx].type === token_1.TokenType.GCP || this.tokens[nextIdx].type === token_1.TokenType.Azure)) {
    ...
    parseMap() {
        ...
        while (!this.check(token_1.TokenType.RBrace) && !this.isAtEnd()) {
            ...
            else if (this.check(token_1.TokenType.String)) {
    ...
    isArrayLiteralStart() {
        ...
        if (nextToken.type === token_1.TokenType.Symbol) {
        ...
        return nextToken.type === token_1.TokenType.Variable || nextToken.type === token_1.TokenType.Number || nextToken.type === token_1.TokenType.String || nextToken.type === token_1.TokenType.RBracket || nextToken.type === token_1.TokenType.LBracket || nextToken.type === token_1.TokenType.LParen || nextToken.type === token_1.TokenType.LBrace || nextToken.type === token_1.TokenType.Keyword || nextToken.type === token_1.TokenType.Colon;
    }
    ...
    parseSExpr() {
        ...
        if (opToken.type === token_1.TokenType.Open) {
            op = "open";
        }
        ...
        else if (opToken.type === token_1.TokenType.Variable) {
    ...
    expect(type) {
        if (this.check(type)) {
            ...
            if (type === token_1.TokenType.LParen || type === token_1.TokenType.LBracket || type === token_1.TokenType.LBrace) {
                this.parenStack.push({ type, line: tok.line, col: tok.col });
            }
            else if (type === token_1.TokenType.RParen || type === token_1.TokenType.RBracket || type === token_1.TokenType.RBrace) {
                this.parenStack.pop();
            }
            return tok;
        }
        ...
        let extraHint = "";
        if ((type === token_1.TokenType.RParen || type === token_1.TokenType.RBracket || type === token_1.TokenType.RBrace) && this.parenStack.length > 0) {
            const opening = this.parenStack[this.parenStack.length - 1];
            const openSym = opening.type === token_1.TokenType.LParen ? "(" : opening.type === token_1.TokenType.LBracket ? "[" : "{";
            const wantSym = type === token_1.TokenType.RParen ? ")" : type === token_1.TokenType.RBracket ? "]" : "}";
    ...
    error(message, token) {
        ...
        if (this.parenStack.length > 0 &&
            (token.type === token_1.TokenType.EOF || message.includes("Expected R") || message.includes("Unexpected"))) {
            // 1. 전체 스택 정보 (깊이, 심볼, 위치)
            ...
            const closeSeq = this.parenStack
                .map(o => o.type === token_1.TokenType.LParen ? ")" : o.type === token_1.TokenType.LBracket ? "]" : "}")
                .reverse()
                .join("");
            // 3. 다음 예상 닫는 괄호 (가장 최근 열린 괄호)
            const nextOpening = this.parenStack[this.parenStack.length - 1];
            const nextExpected = nextOpening.type === token_1.TokenType.LParen ? ")" : nextOpening.type === token_1.TokenType.LBracket ? "]" : "}";
            // 4. 힌트 메시지
    ...
    parseTryExpression() {
        ...
        if (this.check(token_1.TokenType.LParen) && this.pos + 1 < this.tokens.length) {
            const nextToken = this.tokens[this.pos + 1];
            // 첫 S-expr이 catch나 finally가 아니면 body 파싱
            if (!(nextToken.type === token_1.TokenType.Symbol && (nextToken.value === "catch" || nextToken.value === "finally"))) {
                bodyExprs.push(this.parseValue());
                // body 파싱 후 다음이 반드시 catch/finally여야 함 (Strict)
                if (this.check(token_1.TokenType.LParen) && this.pos + 1 < this.tokens.length) {
                    const afterBodyToken = this.tokens[this.pos + 1];
                    if (!(afterBodyToken.type === token_1.TokenType.Symbol && (afterBodyToken.value === "catch" || afterBodyToken.value === "finally"))) {
                        const line = afterBodyToken.line || 0;
                        throw new SyntaxError(`Strict try-catch syntax: after body expression, expected 'catch' or 'finally', ` +
                    ...
                    throw new SyntaxError(`Strict try-catch syntax: expected 'catch', 'finally', or ')' after try body. ` +
                        `Got unexpected token at line ${this.peek().line || 0}`);
                }
            }
        }
        const body = bodyExprs.length === 1 ? bodyExprs[0] : (0, ast_1.makeSExpr)("do", bodyExprs);
        const catchClauses = [];
        let finallyBlock;
        while (this.check(token_1.TokenType.LParen) && !this.isAtEnd()) {
            ...
            if (this.pos + 1 < this.tokens.length) {
                ...
                if (nextToken.type === token_1.TokenType.Symbol && nextToken.value === "catch") {
                    ...
                    else if (this.check(token_1.TokenType.Symbol) || this.check(token_1.TokenType.Variable)) {
                        // Format 2 (self-hosted): (catch param body) - bare param without brackets
                        const paramToken = this.advance();
                        variable = paramToken.value;
                        pattern = (0, ast_1.makeVariablePattern)(paramToken.value);
                    }
                    ...
                    const handler = handlerExprs.length === 1 ? handlerExprs[0] : (0, ast_1.makeSExpr)("do", handlerExprs);
                    ...
                    finallyBlock = finallyExprs.length === 1 ? finallyExprs[0] : (0, ast_1.makeSExpr)("do", finallyExprs);
                    this.expect(token_1.TokenType.RParen);
    ...
    parsePage(name, fields, line) {
        ...
        const pageNode = (0, ast_1.makePageNode)(name, title, path, component, metadata, line);
    ...
    parseRoute(name, fields, line) {
        ...
        return (0, ast_1.makeRouteNode)(name, path, method, handler, middleware, validation, line);
    }
    ...
    parseComponent(name, fields, line) {
        ...
        return (0, ast_1.makeComponentNode)(name, render, state, computed, watch, methods, slots, line);
    }
    ...
    parseForm(name, fields, line) {
        ...
        return (0, ast_1.makeFormNode)(name, formFields, validation, submit, handlers, line);
    }
