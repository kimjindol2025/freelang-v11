...
static int fl_strcmp(FLValue a, FLValue b) {
    return strcmp(((FLString*)a.obj)->data, ((FLString*)b.obj)->data);
}
FLValue fl_lt(FLValue a, FLValue b)  {
    if (a.tag==FL_STRING && b.tag==FL_STRING) return fl_bool(fl_strcmp(a,b) <  0);
...
FLValue fl_map_get(FLValue map, FLValue key) {
    ...
    /* keyword 정규화: ":key" -> "key" */
    if (key.tag == FL_STRING) {
        const char* ks = ((FLString*)key.obj)->data;
        if (ks[0] == ':') key = fl_str_val(ks + 1);
    }
...
FLValue substring(FLValue s, FLValue start, FLValue end) {
    if (s.tag == FL_VECTOR) {
        FLVector *v = (FLVector*)s.obj;
        int64_t len = (int64_t)v->len;
        int64_t a = start.tag==FL_INT ? start.i : 0;
        int64_t b = end.tag==FL_INT ? end.i : len;
        if (a < 0) a = 0; if (b > len) b = len; if (a > b) a = b;
        FLValue r = fl_vec_new();
        for (int64_t i = a; i < b; i++) r = fl_vec_push(r, v->data[i]);
        return r;
    }
