...
# 테스트 파일 찾기
TEST_FILES=$(find "${TEST_DIR}" -name "*.test.fl" 2>/dev/null || true)
...
for test_file in $TEST_FILES; do
  ...
  # FL-native 테스트 실행
  if node --stack-size=8192 "${RUNNER}" run "${test_file}" "${test_file}" 2>&1 | grep -q "PASS"; then
    echo "  ✓ $test_name"
...
echo ""
echo "Unit tests: ${TOTAL_PASS} passed, ${TOTAL_FAIL} failed"
