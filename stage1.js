...
function peek_at() { return (() => { let $unknown = ((i >= _fl_length(src)) ? "null" : char_at(src, i)); return $unknown; })(); }
...
function skip_comment() { return (at_end_q(st) ? st : (() => { let $unknown = ((c === "\n") ? advance(st) : skip_comment(advance(st))); return $unknown; })()); }
function skip_ws() { return (at_end_q(st) ? st : (() => { let $unknown = cond([ _fl_is_space_q(c), skip_ws(advance(st)) ], [ (c === ";"), skip_ws(skip_comment(st)) ], [ true, st ]); return $unknown; })()); }
function read_number_body() { return (at_end_q(st) ? emit(st, "Number", acc, line, col) : (() => { let $unknown = cond([ _fl_is_digit_q(c), read_number_body(advance(st), _fl_str(acc, c), has_dot, line, col) ], [ ((c === ".") && (!has_dot)), read_number_body(advance(st), _fl_str(acc, c), true, line, col) ], [ true, emit(st, "Number", acc, line, col) ]); return $unknown; })()); }
...
function read_string_body() { return (at_end_q(st) ? emit(st, "String", acc, line, col) : (() => { let $unknown = cond([ (c === "\""), emit(advance(st), "String", acc, line, col) ], [ (c === "\\"), (() => { let $unknown = read_string_body(advance(st2), _fl_str(acc, translate_esc(c2)), line, col); return $unknown; })() ], [ true, read_string_body(advance(st), _fl_str(acc, c), line, col) ]); return $unknown; })()); }
function read_string() { return (() => { let $unknown = read_string_body(st1, "", line, col); return $unknown; })(); }
function read_symbol_body_kind() { return (at_end_q(st) ? emit(st, kind, acc, line, col) : (() => { let $unknown = (_fl_is_symbol_char_q(c) ? read_symbol_body_kind(advance(st), _fl_str(acc, c), line, col, kind) : emit(st, kind, acc, line, col)); return $unknown; })()); }
function read_symbol() { return read_symbol_body_kind(st, "", _fl_get(st, "line"), _fl_get(st, "col"), "Symbol"); }
function read_variable() { return (() => { let $unknown = read_symbol_body_kind(st1, "", line, col, "Variable"); return $unknown; })(); }
function read_keyword() { return (() => { let $unknown = read_symbol_body_kind(st1, "", line, col, "Keyword"); return $unknown; })(); }
function read_token() { return (() => { let $unknown = (at_end_q(st1) ? st1 : (() => { let $unknown = cond([ (c === "("), emit(advance(st1), "LParen", c, line, col) ], [ (c === ")"), emit(advance(st1), "RParen", c, line, col) ], [ (c === "["), emit(advance(st1), "LBracket", c, line, col) ], [ (c === "]"), emit(advance(st1), "RBracket", c, line, col) ], [ (c === "{"), emit(advance(st1), "LBrace", c, line, col) ], [ (c === "}"), emit(advance(st1), "RBrace", c, line, col) ], [ (c === "\""), read_string(st1) ], [ (c === "$"), read_variable(st1) ], [ (c === ":"), read_keyword(st1) ], [ _fl_is_digit_q(c), read_number(st1) ], [ ((c === "-") && _fl_is_digit_q(peek_at(st1, 1))), read_number_body(advance(st1), "-", false, line, col) ], [ _fl_is_symbol_char_q(c), read_symbol(st1) ], [ true, emit(advance(st1), "Unknown", c, line, col) ]); return $unknown; })()); return $unknown; })(); }
function lex_loop() { return (() => { let $unknown = (at_end_q(st1) ? _fl_get(st1, "tokens") : lex_loop(read_token(st1))); return $unknown; })(); }
...
function deep_equal_map_q() { return (() => { let $unknown = ((!(_fl_length(ka) === _fl_length(kb))) ? false : deep_equal_map_keys_q(a, b, ka, 0)); return $unknown; })(); }
...
function js_name_inner() { return (() => { let $unknown = (js_reserved_q(g) ? _fl_str(g, "_") : g); return $unknown; })(); }
function js_name() { return (_fl_null_q(n) ? "_" : js_name_inner(n)); }
function cg() { return cond([ _fl_null_q(n), "null" ], [ (_fl_get(n, "kind") === "literal"), cg_literal(n) ], [ (_fl_get(n, "kind") === "variable"), js_name(_fl_get(n, "name")) ], [ (_fl_get(n, "kind") === "keyword"), js_esc(_fl_get(n, "name")) ], [ (_fl_get(n, "kind") === "sexpr"), cg_sexpr(n) ], [ (_fl_get(n, "kind") === "block"), cg_block(n) ], [ (_fl_get(n, "kind") === "pattern-match"), cg_match(n) ], [ (_fl_get(n, "kind") === "await"), _fl_str("(await ", cg(_fl_get(n, "argument")), ")") ], [ (_fl_get(n, "kind") === "throw"), _fl_str("(()=>{throw new Error(String(", cg(_fl_get(n, "argument")), "))})()") ], [ true, "null" ]); }
function cg_literal() { return (() => { let $unknown = cond([ (t === "number"), v ], [ (t === "string"), js_esc(v) ], [ (t === "boolean"), (v ? "true" : "false") ], [ (t === "symbol"), cond([ (v === "true"), "true" ], [ (v === "false"), "false" ], [ (v === "null"), "null" ], [ (v === "nil"), "null" ], [ true, js_name(v) ]) ], [ true, "null" ]); return $unknown; })(); }
function cg_block() { return (() => { let $unknown = cond([ (t === "Array"), _fl_str("[", cg_args(_fl_get(_fl_get(n, "fields"), "items")), "]") ], [ (t === "Map"), _fl_str("({", cg_map_entries(_fl_get(n, "fields")), "})") ], [ (t === "FUNC"), cg_func_block(n) ], [ true, "null" ]); return $unknown; })(); }
function cg_map_entries() { return (() => { let $unknown = (_fl_array_q(items_val) ? cg_map_flat_loop(items_val, 0, "") : cg_map_loop([], 0, "")); return $unknown; })(); }
function cg_map_loop_inner() { return (() => { let $unknown = cg_map_loop(entries, (i + 1), ((i === 0) ? pair : _fl_str(acc, ",", pair))); return $unknown; })(); }
function cg_map_loop() { return ((i >= _fl_length(entries)) ? acc : cg_map_loop_inner(entries, i, acc, _fl_get(entries, i))); }
function cg_map_flat_loop_inner() { return (() => { let $unknown = cg_map_flat_loop(items, (i + 2), ((i === 0) ? pair : _fl_str(acc, ",", pair))); return $unknown; })(); }
...
function cg_func_block_inner() { return (() => { let $unknown = _fl_str("const ", name, " = (", ps, ")=>", cg(body), ";"); return $unknown; })(); }
function cg_func_block() { return (() => { let $unknown = cg_func_block_inner(name, fields, _fl_get(fields, "params"), _fl_get(fields, "body")); return $unknown; })(); }
function cg_if() { return _fl_str("(", cg(_fl_get(args, 0)), "?", cg(_fl_get(args, 1)), ":", ((_fl_length(args) >= 3) ? cg(_fl_get(args, 2)) : "null"), ")"); }
function cg_fn() { return (() => { let $unknown = _fl_str("((", ps, ")=>", cg(body), ")"); return $unknown; })(); }
function cg_defn() { return (() => { let $unknown = _fl_str("const ", name, " = (", ps, ")=>", cg(body)); return $unknown; })(); }
function cg_define() { return ((_fl_length(args) === 2) ? _fl_str("const ", extract_name(_fl_get(args, 0)), " = ", cg(_fl_get(args, 1))) : cg_defn(args)); }
function cg_let() { return (() => { let $unknown = _fl_str("((()=>{", bindings, body_js, "})())"); return $unknown; })(); }
function cg_let_1d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = cg_let_1d(items, (i + 2), _fl_str(acc, "let ", name, "=", val, ";")); return $unknown; })()); }
function cg_let_2d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = cg_let_2d(items, (i + 1), _fl_str(acc, "let ", name, "=", val, ";")); return $unknown; })()); }
function cg_do() { return ((_fl_length(args) === 0) ? "null" : _fl_str("(()=>{", cg_do_body(args, 0, ""), "})()")); }
function cg_do_body() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = cg_do_body(args, (i + 1), _fl_str(acc, sep, c, ";")); return $unknown; })()); }
function cg_cond() { return cg_cond_loop(args, 0); }
function cg_cond_loop() { return loop([ j, (_fl_length(args) - 1), acc, "null" ], ((j < 0) ? acc : (() => { let $unknown = recur((j - 1), _fl_str("(", test, "?", result, ":", acc, ")")); return $unknown; })())); }
function cg_and() { return ((_fl_length(args) === 0) ? "true" : _fl_str("(", and_loop(args, 0, ""), ")")); }
function and_loop() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = and_loop(args, (i + 1), ((i === 0) ? c : _fl_str(acc, "&&", c))); return $unknown; })()); }
function cg_or() { return ((_fl_length(args) === 0) ? "false" : _fl_str("(", or_loop(args, 0, ""), ")")); }
function or_loop() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = or_loop(args, (i + 1), ((i === 0) ? c : _fl_str(acc, "||", c))); return $unknown; })()); }
...
function cg_throw() { return (() => { let $unknown = _fl_str("(()=>{throw new Error(String(", m, "))})()"); return $unknown; })(); }
function cg_try() { return (() => { let $unknown = _fl_str("(()=>{try{return ", body, ";}catch(", evar, "){return ", ebody, ";}})()"); return $unknown; })(); }
function extract_catch() { return (() => { let $unknown = {}; return $unknown; })(); }
function cg_while() { return (() => { let $unknown = _fl_str("(()=>{while(", cond, "){", body, "}})()"); return $unknown; })(); }
function while_body() { return ((i >= _fl_length(args)) ? acc : while_body(args, (i + 1), _fl_str(acc, cg(_fl_get(args, i)), ";"))); }
function cg_loop() { return (() => { let $unknown = _fl_str("((()=>{", bindings, "while(true){let __r=(()=>{", body_js, "})();", "if(__r&&__r.__recur){[", names, "]=__r.a;continue;}", "return __r;}})())"); return $unknown; })(); }
function loop_inits_1d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = loop_inits_1d(items, (i + 2), _fl_str(acc, "let ", extract_name(k), "=", cg(v), ";")); return $unknown; })()); }
function loop_inits_2d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = loop_inits_2d(items, (i + 1), _fl_str(acc, "let ", extract_name(_fl_get(pit, 0)), "=", cg(_fl_get(pit, 1)), ";")); return $unknown; })()); }
function loop_names_1d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = loop_names_1d(items, (i + 2), ((_fl_length(acc) === 0) ? n : _fl_str(acc, ",", n))); return $unknown; })()); }
function loop_names_2d() { return ((i >= _fl_length(items)) ? acc : (() => { let $unknown = loop_names_2d(items, (i + 1), ((_fl_length(acc) === 0) ? n : _fl_str(acc, ",", n))); return $unknown; })()); }
function cg_recur() { return _fl_str("{__recur:true,a:[", cg_args(args), "]}"); }
function cg_match() { return (() => { let $unknown = _fl_str("((__v)=>{", cases_js, "return ", _default, ";})(", val, ")"); return $unknown; })(); }
function match_cases_loop() { return ((i >= _fl_length(cases)) ? acc : (() => { let $unknown = match_cases_loop(cases, (i + 1), _fl_str(acc, "if(", test, "){", inner, "}")); return $unknown; })()); }
function pattern_test() { return (() => { let $unknown = cond([ (k === "wildcard-pattern"), "true" ], [ (k === "variable-pattern"), "true" ], [ (k === "literal-pattern"), _fl_str("(", v, "===", literal_to_js(_fl_get(pat, "type"), _fl_get(pat, "value")), ")") ], [ (k === "or-pattern"), or_pattern_test(_fl_get(pat, "patterns"), v, 0, "") ], [ (k === "range-pattern"), _fl_str("(", v, ">=", _fl_get(pat, "min"), "&&", v, "<=", _fl_get(pat, "max"), ")") ], [ true, "false" ]); return $unknown; })(); }
...
function or_pattern_test() { return ((i >= _fl_length(pats)) ? ((_fl_length(acc) === 0) ? "false" : _fl_str("(", acc, ")")) : (() => { let $unknown = or_pattern_test(pats, v, (i + 1), ((_fl_length(acc) === 0) ? t : _fl_str(acc, "||", t))); return $unknown; })()); }
function pattern_bindings() { return (() => { let $unknown = cond([ (k === "variable-pattern"), _fl_str("let ", js_name(_fl_get(pat, "name")), "=", v, ";") ], [ true, "" ]); return $unknown; })(); }
function cg_compose() { return _fl_str("((__x)=>", compose_nest(args, (_fl_length(args) - 1), "__x"), ")"); }
function compose_nest() { return ((i < 0) ? acc : (() => { let $unknown = compose_nest(args, (i - 1), _fl_str(f, "(", acc, ")")); return $unknown; })()); }
function cg_pipe() { return _fl_str("((__x)=>", pipe_nest(args, 0, "__x"), ")"); }
function pipe_nest() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = pipe_nest(args, (i + 1), _fl_str(f, "(", acc, ")")); return $unknown; })()); }
function cg_thread_first() { return (() => { let $unknown = thread_first_loop(args, 1, init); return $unknown; })(); }
function thread_first_loop() { return ((i >= _fl_length(args)) ? acc : thread_first_loop(args, (i + 1), thread_apply(_fl_get(args, i), acc, true))); }
function cg_thread_last() { return (() => { let $unknown = thread_last_loop(args, 1, init); return $unknown; })(); }
function thread_last_loop() { return ((i >= _fl_length(args)) ? acc : thread_last_loop(args, (i + 1), thread_apply(_fl_get(args, i), acc, false))); }
function thread_apply() { return cond([ (_fl_get(step, "kind") === "sexpr"), (() => { let $unknown = (fst ? _fl_str(op, "(", acc, ((_fl_length(a) === 0) ? "" : _fl_str(",", cg_args(a))), ")") : _fl_str(op, "(", ((_fl_length(a) === 0) ? "" : _fl_str(cg_args(a), ",")), acc, ")")); return $unknown; })() ], [ true, _fl_str(_fl_str(cg(step)), "(", acc, ")") ]); }
function cg_call_form() { return (() => { let $unknown = _fl_str(fn, "(", cg_args(_fl_rest), ")"); return $unknown; })(); }
function cg_async() { return _fl_str("(async()=>{", cg_do_body(args, 0, ""), "})()"); }
function cg_await() { return _fl_str("(await ", cg(_fl_get(args, 0)), ")"); }
function cg_defstruct() { return (() => { let $unknown = _fl_str("const ", name, " = (", params, ")=>({__struct:\"", name, "\",", init, "})"); return $unknown; })(); }
function struct_field_list() { return ((i >= _fl_length(fs)) ? acc : (() => { let $unknown = struct_field_list(fs, (i + 1), ((_fl_length(acc) === 0) ? n : _fl_str(acc, ",", n))); return $unknown; })()); }
function struct_init_list() { return ((i >= _fl_length(fs)) ? acc : (() => { let $unknown = struct_init_list(fs, (i + 1), ((_fl_length(acc) === 0) ? n : _fl_str(acc, ",", n))); return $unknown; })()); }
function cg_args() { return args_loop(args, 0, ""); }
function args_loop() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = args_loop(args, (i + 1), ((i === 0) ? c : _fl_str(acc, ",", c))); return $unknown; })()); }
...
function binop_loop() { return ((i >= _fl_length(args)) ? acc : (() => { let $unknown = binop_loop(args, (i + 1), ((i === 0) ? c : _fl_str(acc, op, c)), op); return $unknown; })()); }
function cg_pair() { return _fl_str("(", cg(_fl_get(args, 0)), op, cg(_fl_get(args, 1)), ")"); }
function cg_sub() { return ((_fl_length(args) === 1) ? _fl_str("(-", cg(_fl_get(args, 0)), ")") : _fl_str("(", cg(_fl_get(args, 0)), "-", cg(_fl_get(args, 1)), ")")); }
...
function prelude_list() { return _fl_str("const _fl_first = (l) => (l==null || l.length===0 ? null : l[0]);", "const _fl_last = (l) => (l==null || l.length===0 ? null : l[l.length-1]);", "const _fl_rest = (l) => (l==null || l.length===0 ? [] : l.slice(1));", "const _fl_append = (a, b) => (Array.isArray(b) ? [...a, ...b] : [...a, b]);", "const _fl_slice = (l, a, b) => l.slice(a, b);", "const _fl_map = (f, l) => l.map(x => f(x));", "const _fl_filter = (f, l) => l.filter(x => f(x));", "const _fl_reduce = (f, init, l) => l.reduce((acc, x) => f(acc, x), init);", "const _fl_find = (f, l) => (l.find(x => f(x)) ?? null);", "const _fl_every = (f, l) => l.every(x => f(x));", "const _fl_some = (f, l) => l.some(x => f(x));", "const _fl_sort = (l, cmp) => [...l].sort(cmp || ((a,b)=>a<b?-1:a>b?1:0));", "const _fl_reverse = (l) => [...l].reverse();", "const _fl_flatten = (l) => l.flat(Infinity);", "const _fl_distinct = (l) => [...new Set(l)];", "const _fl_range = (a, b, s) => { let r=[]; let step=s||1; for(let i=a;i<b;i+=step)r.push(i); return r; };", "const _fl_take = (n, l) => l.slice(0, n);", "const _fl_drop = (n, l) => l.slice(n);"); }
...
function runtime_prelude() { return __fl_reduce((() => _fl_str(acc, part)), "", prelude_parts()); }
function fl__gtjs_code() { return __fl_reduce((() => _fl_str(acc, cg(node), ";\n")), "", ast); }
function fl__gtjs_with_prelude() { return (() => { let $unknown = (() => { let $unknown = _fl_str(p, "\n", c); return $unknown; })(); return $unknown; })(); }
function compile_file() { return (() => { let $unknown = file_write(output, js); return $unknown; })(); }
...
function str_count_loop() { return (() => { let $unknown = ((idx < 0) ? acc : str_count_loop(s, sub, _plus(pos, idx, _fl_length(sub)), (acc + 1))); return $unknown; })(); }
function csv_parse_line() { return split(line, ","); }
function csv_parse() { return (() => { let $unknown = csv_parse_loop(lines, 0, []); return $unknown; })(); }
...
function to_query_loop() { return ((i >= _fl_length(ks)) ? join(acc, "&") : (() => { let $unknown = to_query_loop(ks, obj, (i + 1), _fl_append(acc, _fl_str(k, "=", v))); return $unknown; })()); }
...
function hash_fnv1a() { return (() => { let $unknown = hash_fnv_loop(_fl_str(s), 0, fnv_offset); return $unknown; })(); }
...
function hash_consistent() { return (() => { let $unknown = (((h * seed) + seed) % 2147483647); return $unknown; })(); }
...
function time_ago() { return (() => { let $unknown = cond([ (s < 60), _fl_str(s, "s ago") ], [ (m < 60), _fl_str(m, "m ago") ], [ (h < 24), _fl_str(h, "h ago") ], [ true, _fl_str(d, "d ago") ]); return $unknown; })(); }
...
function str_pad_left() { return (() => { let $unknown = ((diff <= 0) ? s : _fl_str(str_repeat(pad, diff), s)); return $unknown; })(); }
function str_pad_right() { return (() => { let $unknown = ((diff <= 0) ? s : _fl_str(s, str_repeat(pad, diff))); return $unknown; })(); }
...
__fl_print("═══════════════════════════════════════════\n")
(() => { let $unknown = ((_fl_null_q(input) || _fl_null_q(output)) ? __fl_print("Usage: node bootstrap.js run self/main.fl <input.fl> <output.js>") : (() => { let $unknown = (() => { let $unknown = (() => { let $unknown = (() => { let $unknown = (() => { __fl_print(_fl_str("📖 ", input, " → ", output)); __fl_print(_fl_str("   Lexed: ", _fl_length(tokens), " tokens")); __fl_print(_fl_str("   Parsed: ", _fl_length(ast), " nodes")); __fl_print(_fl_str("   Generated: ", _fl_length(js), " bytes")); file_write(output, js); return __fl_print("✨ Success!"); })(); return $unknown; })(); return $unknown; })(); return $unknown; })(); return $unknown; })()); return $unknown; })()
module.exports = {
