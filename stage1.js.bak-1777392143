...
function cg_block() { return (() => { let unknown = ((t === "FUNC") ? cg_func_block(n) : cg_block_dispatch(t, fields)); return unknown; })(); }
...
function cg_defn() { return (() => { let unknown = _fl_str(warning, (fl_reserved_q(name) ? "\n" : ""), "const ", name, " = (", ps, ")=>", cg(body)); return unknown; })(); }
...
function cg_let_binding() { return (() => { let unknown = cond([ (kind === "variable"), _fl_str("let ", js_name(_fl_get(pat_node, "name")), "=", val_js, ";") ], [ ((kind === "block") && (type === "Map")), (() => { let unknown = _fl_str(((val_js === tmpvar) ? "" : _fl_str("let ", tmpvar, "=", val_js, ";")), pat_bindings); return unknown; })() ], [ ((kind === "block") && (type === "Array")), (() => { let unknown = (_fl_null_q(first_pat) ? "" : _fl_str("let ", tmpvar, "=", val_js, "[0];", cg_let_binding(first_pat, tmpvar, tmpvar_next))); return unknown; })() ], [ true, _fl_str("let ", extract_name(pat_node), "=", val_js, ";") ]); return unknown; })(); }
function cg_let_1d() { return ((i >= _fl_length(items)) ? acc : (() => { let unknown = cg_let_1d(items, (i + 2), _fl_str(acc, cg_let_binding(pat_n, val, tmpvar)), (counter + 1)); return unknown; })()); }
function cg_let_2d() { return ((i >= _fl_length(items)) ? acc : (() => { let unknown = cg_let_2d(items, (i + 1), _fl_str(acc, cg_let_binding(k_n, val, tmpvar)), (counter + 1)); return unknown; })()); }
...
function get_clause_items() { return (() => { let unknown = (_fl_array_q(items) ? items : [_fl_get(fields, 0), _fl_get(fields, 1)]); return unknown; })(); }
...
function struct_pattern_test() { return (nil_q(_fl_keys) ? (() => { let unknown = struct_pattern_test(fields, v, 0, ks, ""); return unknown; })() : ((i >= _fl_length(_fl_keys)) ? ((_fl_length(acc) === 0) ? _fl_str("(", v, "!==null&&typeof ", v, "===\"object\")") : _fl_str("(", v, "!==null&&typeof ", v, "===\"object\"&&", acc, ")")) : (() => { let unknown = struct_pattern_test(fields, v, (i + 1), _fl_keys, ((_fl_length(acc) === 0) ? t : _fl_str(acc, "&&", t))); return unknown; })())); }
...
function struct_pattern_bindings_from_entries() { return ((i >= _fl_length(entries)) ? acc : (() => { let unknown = struct_pattern_bindings_from_entries(entries, v, (i + 1), _fl_str(acc, binding)); return unknown; })()); }
function cg_map_pattern_bindings_from_entries() { return ((i >= _fl_length(entries)) ? acc : (() => { let unknown = cg_map_pattern_bindings_from_entries(entries, v, (i + 1), _fl_str(acc, binding)); return unknown; })()); }
function struct_pattern_bindings() { return (nil_q(_fl_keys) ? (() => { let unknown = (_fl_empty_q(ks) ? struct_pattern_bindings_from_entries(map_entries(fields), v, 0, "") : struct_pattern_bindings(fields, v, 0, ks, "")); return unknown; })() : ((i >= _fl_length(_fl_keys)) ? acc : (() => { let unknown = struct_pattern_bindings(fields, v, (i + 1), _fl_keys, _fl_str(acc, field_bindings)); return unknown; })())); }
