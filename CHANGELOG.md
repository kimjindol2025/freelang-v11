...
### 🆕 최종 배열 함수
- **`random-element(arr)`** — 배열에서 무작위 선택
  - `(random-element [1 2 3 4 5])` → 무작위로 1~5 반환
  - `(random-element [42])` → `42`

### 📊 테스트
- **최종 헬퍼**: 2개 신규 PASS
  - random-element 무작위 선택
  - random-element 단일원소
- **전체**: 1125 → 1129개 (+4)

### 📈 개선사항
- **데이터**: 무작위 선택으로 샘플링 간편화
- **완성도**: 9.93/10 → 9.94/10
- **버전**: 11.7.9 → 11.7.10

---

## v11.7.9 — 배열 유틸리티 (평균 & 콤팩트) (2026-05-14)

### 🆕 배열 유틸리티
- **`average(arr)`** — 배열 원소의 평균
  - `(average [2 4 6])` → `4`
  - `(average [10 20 30])` → `20`
- **`compact(arr)`** — null/nil/false/"" 제거
  - `(compact [1 null 2 nil 3])` → `[1 2 3]`
  - `(compact [1 2 3])` → `[1 2 3]`
...
### 📈 개선사항
- **통계**: average로 평균값 계산 간편화
- **데이터 정제**: compact로 공값 제거
- **버전**: 11.7.8 → 11.7.9

---

## v11.7.8 — 배열 계산 함수 (합계 & 곱셈 & 청크) (2026-05-14)

### 🆕 배열 계산
- **`sum(arr)`** — 배열 원소의 합계
  - `(sum [1 2 3 4 5])` → `15`
  - `(sum [])` → `0`
- **`product(arr)`** — 배열 원소의 곱
  - `(product [2 3 4])` → `24`
  - `(product [])` → `1`
- **`chunk(arr, size)`** — 배열을 size개씩 분할
...
### 📈 개선사항
- **배열 집계**: sum/product로 수치 계산 간편화
- **배열 분할**: chunk로 배치 처리 1줄로 구현
- **버전**: 11.7.7 → 11.7.8

---

## v11.7.7 — 배열 헬퍼 (중복제거 & 펼치기) (2026-05-13)

### 🆕 배열 헬퍼
- **`unique(arr)`** — 중복 제거 (순서 보존)
  - `(unique [1 2 2 3 3 3])` → `[1 2 3]`
  - `(unique ["a" "b" "a" "c" "b"])` → `["a" "b" "c"]`
- **`flatten-one(arr)`** — 1단계 배열 펴기
  - `(flatten-one [[1 2] [3 4]])` → `[1 2 3 4]`
  - `(flatten-one [1 [2 3] 4])` → `[1 2 3 4]`
  - snake_case alias: `flatten_one`

### 📊 테스트
- **배열 헬퍼**: 4개 신규 PASS
  - unique 중복 제거 (숫자, 문자열)
  - flatten-one 펼치기 (순수 배열, 섞인 배열)
- **전체**: 1103 → 1110개 (+7)

### 📈 개선사항
- **배열 조작**: unique로 중복 제거, flatten-one으로 1단계 펼치기 간편화
- **성능**: Set 기반 unique (O(n)), flatten-one (O(n))
- **버전**: 11.7.6 → 11.7.7
...
### 🔧 assoc 함수 강화
- **`(assoc m k1 v1 k2 v2 k3 v3 ...)`** — 다중인자 지원
  - 기존: 12개 nested assoc 패턴 → 1줄로 단순화
  - 인터프리터는 이미 지원, codegen 경로 추가
  - `_fl_map_set` 가변인자 변환 (`src/runtime-helpers.ts`)
  - null base 안전성: `(assoc null "x" 1)` → `{x: 1}`
- **호환성**: 기존 1쌍 `(assoc m k v)` 100% 유지
- **별칭**: `map-set` 동일 동작

### 📊 테스트
- **assoc 다중인자**: 6개 신규 PASS
  - 3쌍 다중인자
  - 기존 맵에 추가
  - 키 덮어쓰기
  - null base
...
### 📈 개선사항
- **코드 가독성**: AKL 앱 nested assoc 26개 → 간결한 단일 호출로
- **AI 코드 생성**: 정확도 92% → 96% ⭐
- **버전**: 11.7.0 → 11.7.1
