# v11.5.x 언어 통일 — 제안서 (PROPOSAL)

> **상태**: 제안서. 일부 (kebab alias)는 v11.5.0에 *구현됨*, 나머지는 미구현.
> **목표**: v11의 디자인 결함 44개를 통일된 언어 규칙으로 fix
> **버전**: v11.5.0 (kebab alias 시작) → v11.5.1 (시그니처 통일) [제안]
> **타임라인**: 3개월 (제안)
> **호환성**: alias 유지 + `freelang-migrate` 도구
...
|------|------|
| 1. 인자 순서 통일 | ❌ 제안만 (구현 X) |
| 2. **kebab-case alias** | ✅ **v11.5.0 구현됨** (400개) |
| 3. 술어 `?` suffix | △ 부분 (file-exists? 등) |
| 4. HTTP 응답 분리 함수 | ❌ 제안만 |
| 5. `$` 접두사 선택 | ❌ 제안만 |
