...
| 함수 | 동작 |
|------|------|
| `file-append-line` | `(file-append-line path text)` — text + `\n` 자동 개행 추가 |

### MISTAKES-100 재분류 (이미 동작 확인 → covered)
| # | 항목 | 이유 |
|---|------|------|
| #21 | v10 let 단일 괄호 | `(let [x 10 y 20] ...)` 이미 동작 |
| #23 | 단일 바인딩 단일 괄호 | `(let [u "alice"] ...)` 이미 동작 |
| #26 | [FUNC] deprecated | 파서에서 deprecation 경고 이미 출력 |
| #28 | $ 파라미터 누락 | `defn f [a b]` — $ 접두사 선택사항 이미 동작 |
| #29 | fn $ 누락 | `(fn [x] ...)` — $ 접두사 선택사항 이미 동작 |

커버리지: 70% → **76%** | Tests: 926/926 | L2 SHA: 47407b90
...
**마일스톤**: 에러 메시지 강화 + 스택 트레이스 — 완성도 9.5/10 달성

### 🐛 개선사항

#### 에러 메시지 (Phase B)
- **스택 트레이스**: 함수 호출 체인 명확화
- **FreeLangError 인터페이스 확장**: stack 필드 추가
- **formatError 함수**: 호출 스택 출력 (들여쓰기 계층 구조)

#### 함수 동작 일관성
- **reverse**: 비배열 입력 → [] 반환 (경로 1,2 통일)

**완성도**: 9.2/10 → **9.5/10** ✨

## [11.3.5] - 2026-05-03

**마일스톤**: 소스 코드 경로 통합 완료 — 15개 함수 수정 (누적 89개)

### 🐛 버그 수정 (eval-builtins.ts 소스 수정)

#### 문자열/출력 함수
- `str/concat`: String() → toDisplay() 통합
- `print`: String() → toDisplay() 통합
...
#### Map 데이터 손실 (P0)
- **`dissoc` Map 데이터 손실**: Map 인스턴스 보존, 기존 항목 유지

#### 함수값 호출 불가 (P1)
- **`memoize`/`once`/`tap`/`pipeline_run`/`retry` 함수값 미지원**: callFunctionValue 래퍼 사용
- **컬렉션 모듈 리팩토링**: interpreter 컨텍스트 전달로 고차함수 정상 작동

#### 타입 불일치 (P2)
- **`first` 문자열 미지원**: 배열과 동일하게 첫 문자 반환
- **`rest` 문자열 미지원 + null 오류**: 배열과 동일하게 작동, null → null

#### 파싱 오류 (P3)
- **`inc`/`dec` NaN 처리 안 됨**: 입력 "abc" → null (NaN 체크)
