...
const map_entries = (m) => _fl_map(_fl_keys(m), ((k) => [k, _fl_get(m, k)]))
const prompt_template = (template, vars) => _fl_reduce(map_entries(vars), ((acc, _arg0) => ((() => { let key = _arg0[0]; let val = _arg0[1]; return _fl_replace(acc, _fl_str("{", key, "}"), _fl_str(val)); })())), template)
